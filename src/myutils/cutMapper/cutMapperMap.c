/* cutMapperMap - Maps reads in FastQ file using and indexed genome and makes output into jfd format. */
#include "common.h"
#include "linefile.h"
#include "hash.h"
#include "options.h"
...
/* Explain usage and exit. */
{
  ...
  "cutMapperMap - Maps reads in FastQ file using and indexed genome and makes output into jfd format\n"
  "usage:\n"
  "   cutMapperMap indexFolder file.fastQ mappedreads.txt\n"
  "options:\n"
...
void reParseFastqFileAndMap(char *inFastq, khash_t(hashPos_t) **hReads, replist_t *replistp, char *outFile, char **chromNames)
/* hashFastqFile - Parse file and map reads using the hash and repeat table */
{
  ...
  char *seqName, *seq, *seqString, *qName, *qual;
  int line = 0;
  //int numKmers = 0;
  int numUni = 0;
  int numRep = 0;
  ...
  while ( lineFileNext(lf, &seqName, NULL)){
	...
	if(kh_value(hReads[Prefix], khit).chr==255){//Non uniquely mappable //255 Use 254 for debugging
	  ...
	  if(kv_size(*repvecp)<=10){
	    ++numRep;
	    if(applySensitivity){	      
	      aux=resample_reads(sens,repvecp->a,kv_size(*repvecp));
	      fprintf(f,"%s\t%s\t%d\t%c\t%s\t%d\n",seqName,
		      chromNames[aux.chr],
		      abs(aux.pos),
		      aux.pos<0 ? '-' : '+',
		      seqString,(int)kv_size(*repvecp));	      
	    }
	    /*	    else{
	      fprintf(f,"%s",seqName);	  	      
...
void cutMapperMap(char *inFastq, char *indexFolder, char *uniFile)
/* cutMapperMap - Maps reads in FastQ file using and indexed genome and makes output into jfd format. */
{
  //  FILE *f;
  ...
  //for(j=57;j<58;j++){ //256
   for(j=0;j<256;j++){ //256
    ...
    verbose(1,"# Opened repeats for index %d in %f seconds (%f total)\n",j,
	    (double)(clock() - t)/CLOCKS_PER_SEC,
	    (double)(clock() - t0)/CLOCKS_PER_SEC);
  ...
  reParseFastqFileAndMap(inFastq ,hMapped,&replistMapped,uniFile,chromNames);
...
/* Process command line. */
{
  ...
  if (argc != 4)
    usage();
  cutMapperMap(argv[2],argv[1],argv[3]);
  return 0;
}
