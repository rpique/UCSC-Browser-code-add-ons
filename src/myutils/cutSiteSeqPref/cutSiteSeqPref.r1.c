...
/* cutSiteSeqPref - Finds DNase-I cut-site preference.. */
{
  ...
  unsigned int kmerCutsRev[100][maxKmerNum];
  unsigned int kmerCutsFor[100][maxKmerNum];
  ...
  for(iChr=1;iChr<xblCuts->count;iChr++){
	...
	/*	if((xblMapp->vec[(iChr)].a[k]==1)){
	  cM=cM+1;
	  cF=cF+xblCuts->vec[(iChr<<1)].a[k];
	  if(k>window)
	    cF=cF-
	*/
	/*
	if(xblMapp->vec[(iChr)].a[k]==1) 
	  fprintf(outFile,"%s %d %d %lx M%d cF%d cR%d\n",
		  xblCuts->names[iChr],j,k,
		  ForKmer&cutMask,xblMapp->vec[(iChr)].a[k],
		  xblCuts->vec[(iChr<<1)].a[k],
		  xblCuts->vec[(iChr<<1)+1].a[k]);
	*/

	/* I don't know if the best would be xblMapp at -19,-20,-21 for the - strand? */
	if((xblMapp->vec[(iChr)].a[k]==1) && (xblMapp->vec[(iChr)].a[k-20]==1)){
	  int kmer=ForKmer&cutMask;
	  kmerOcc[kmer]++;
	  if(xblCuts->vec[(iChr<<1)].a[k] < 100)
	    kmerCutsFor[xblCuts->vec[(iChr<<1)].a[k]][kmer]++;
	  else
	    kmerCutsFor[99][kmer]++;
	  if(xblCuts->vec[(iChr<<1)+1].a[k-1] < 100)
	    kmerCutsRev[xblCuts->vec[(iChr<<1)+1].a[k-1]][kmer]++;
	  else
	    kmerCutsRev[99][kmer]++;	    	    
	}	
	// --- //	 hashKmerCheckRepeatPos(ForKmer,h,replistp,chr,);  //
  ...
  for(k=0;k<maxKmerNum;++k){
    ...
    for(j=0;j<100;j++)
      fprintf(outFile,"\t%d",kmerCutsFor[j][k]);
    for(j=0;j<100;j++)
      fprintf(outFile,"\t%d",kmerCutsRev[j][k]);    
